...
/*!
    ...
    @return  None (void).
*/
void Adafruit_Image::draw(int16_t x, int16_t y)
{
  if (format == IMAGE_1)
  {
                                              ...
                                              int16_t y, boolean transact)
{
  ...
  return coreBMP(filename, tftbuf, x, y, NULL, transact);
}
                                              ...
                                              Adafruit_Image &img)
{
  ...
  return coreBMP(filename, NULL, 0, 0, &img, false);
}
...
{ // SD & TFT sharing bus, use transactions
  ...
  if (readLE16() == 0x4D42)
  {                            // BMP signature
    int fileSize = readLE32(); // Read & ignore file size
    ...
    if (headerSize > 12)
    {
      ...
      int rawBitmapDatasize = readLE32(); // Raw bitmap data size; ignore
      Serial.print("rawBitmapDatasize:");
      Serial.println(rawBitmapDatasize);
      int horizontalResolution = readLE32(); // Horizontal resolution, ignore
      Serial.print("horizontalResolution:");
      Serial.println(horizontalResolution);
      int verticalResolution = readLE32(); // Vertical resolution, ignore
      Serial.print("verticalResolution:");
      Serial.println(verticalResolution);
      colors = readLE32(); // Number of colors in palette, or 0 for 2^depth

      Serial.print("colors:");
      Serial.println(colors);
      int numberOfColorsUsed = readLE32(); // Number of colors used (ignore)
      Serial.print("numberOfColorsUsed:");
      Serial.println(numberOfColorsUsed);
